...
@section('PageTitle', 'Menü İçerikleri')
        ...
        <div class="col-xl-12">
            ...
            <div class="card card-dashed" style="border-style: dotted">
                <div class="card-header ribbon ribbon-start ribbon-clip">
                    <div class="ribbon-label bg-primary-active fs-6" style="padding: 10px 15px;"><span class="d-flex text-white fw-bolder fs-6">Menü İçerikleri Tablosu</span></div>
                    <h3 class="card-title"></h3>
                    <div class="card-toolbar">
                        <a href="{{ route('DigitalMenuContents.Sort.Index') }}" class="btn btn-light-primary btn-icon btn-sm">
                            <i class="fas fa-sort"></i>
                        </a>
                        <a href="{{ route('DigitalMenuContents.Create') }}" class="btn btn-light-success btn-icon btn-sm ms-2">
...
@section('PageCustomJS')
    <script>
        var table, dt;
        var editUrl = '{{ route("DigitalMenuContents.Edit", ":id") }}';
        ...
        var initDatatable = function () {
            dt = $("#datatable").DataTable({
                ...
                info: false,
                ajax: {
                    url : "{{ route('DigitalMenuContents.Index') }}",
                },
                language: {
    ...
    <script type="text/javascript">
        function deleteData(btn) {
            Swal.fire({
                ...
                if (result.value) {
                    $.ajax({
                        url: "contents/"+btn.getAttribute('data-id'),
                        type: 'DELETE',
                        data: {

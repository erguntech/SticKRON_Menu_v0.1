...
@section('PageTitle', 'Müşteri Yönetimi')
        ...
        <div class="col-xl-12">
            ...
            <div class="card card-dashed" style="border-style: dotted">
                <div class="card-header ribbon ribbon-start ribbon-clip">
                    <div class="ribbon-label bg-primary-active fs-6" style="padding: 10px 15px;"><span class="d-flex text-white fw-bolder fs-6">Müşteri Tablosu</span></div>
                    <h3 class="card-title"></h3>
                    <div class="card-toolbar">
                        <a href="{{ route('Clients.Create') }}" class="btn btn-light-success btn-icon btn-sm">
...
@section('PageCustomJS')
    ...
    <script>
        ...
        function assignCredits(btn) {
            Swal.fire({
                title: 'Plakalık Stand Kredisi Yükleme',
                html: `<input id="creditInput" type="text" class="form-control" placeholder="Yüklenecek Kredi Miktarı">`,
                icon: 'info',
                ...
                preConfirm: () => {
                    const input = document.getElementById('creditInput').value.trim();
                    if (input === "" || !containsOnlyDigits(input)) {
                        Swal.fire({
                            icon: 'error',
                            title: 'Hatalı Giriş',
                ...
                if (result.isConfirmed) {
                    const assignedCredits = result.value;

                    $.ajax({
                        url: '/clients/assigncredits',
                        type: 'POST',
                        data: {
                        ...
                        success: function (data) {
                            Swal.fire({
                                ...
                                html: 'Kredi başarı ile aktarıldı.<br><br><strong class="text-warning">Önceki Kredi:</strong> '+ data.oldCredits.toLocaleString('tr-TR') +'<br><strong class="text-primary">Aktarılan Kredi: </strong>'+ data.creditsToAdd.toLocaleString('tr-TR') +'<br><strong class="text-success">Yeni Kredi: </strong>'+ data.totalCredits.toLocaleString('tr-TR'),
                                confirmButtonText: 'Tamam',
                                allowOutsideClick: false,
                                customClass: {
                                    confirmButton: 'btn btn-success',
    ...
    <script type="text/javascript">
        function deleteData(btn) {
            Swal.fire({
                ...
                if (result.value) {
                    $.ajax({
                        url: "clients/"+btn.getAttribute('data-id'),
                        type: 'DELETE',
                        data: {

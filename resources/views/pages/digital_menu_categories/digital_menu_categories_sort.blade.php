...
@section('PageTitle', 'Menü Kategorisi Sıralaması')
            ...
            @if (count($digitalMenuCategories) < 1)
                <div class="alert alert-info d-flex align-items-center p-5 mt-4">
                    ...
                    <div class="d-flex flex-column">
                        ...
                        <span>- Sisteme eklenmiş herhangi bir kayıt bulunamadı. Lütfen önce <a href="{{ route('DigitalMenuCategories.Create') }}" class="text-success">"Menü Kategorisi Ekle"</a> kısmından sisteme yeni bir kayıt ekleyiniz.</span>
                    </div>
                </div>
            @else
                <div class="card shadow-sm">
                    <div class="card-header ribbon ribbon-start ribbon-clip">
                        <div class="ribbon-label bg-warning fs-6" style="padding: 10px 15px;"><span class="d-flex text-white fw-bolder fs-6">Menü Kategorisi Sıralaması Bilgileri</span></div>
                        <h3 class="card-title"></h3>
                        <div class="card-toolbar"></div>
                    </div>
                    <div class="card-body">
                        <form action="{{ route('DigitalMenuCategories.Sort.Update') }}" id="editForm" enctype="multipart/form-data" method="POST" autocomplete="off">
                            @csrf
                            @method('POST')
                            <div class="row" id="sortList">
                                @foreach($digitalMenuCategories as $digitalMenuCategory)
                                    <div class="sort-item alert alert-{{ ($digitalMenuCategory->is_active) ? 'primary' : 'warning' }} d-flex align-items-center p-3" data-id="{{ $digitalMenuCategory->id }}">
                                        <span class="position-badge badge badge-{{ ($digitalMenuCategory->is_active) ? 'primary' : 'warning' }} me-2">1</span>
                                        Kategori Adı: {{ $digitalMenuCategory->category_name }}
...
@section('PageCustomJS')
    @if (count($digitalMenuCategories) > 0)
        <script>
